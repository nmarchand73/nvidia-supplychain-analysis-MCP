...
def get_earnings_calendar(tickers):
    ...
    for ticker in tickers:
        try:
            ...
            # Get earnings dates - handle both DataFrame and dict formats
            try:
                calendar = stock.calendar
                if calendar is not None:
                    # Check if it's a DataFrame
                    if isinstance(calendar, pd.DataFrame):
                        # Extract earnings date from DataFrame
                        if 'Earnings Date' in calendar.index:
                            earnings_date = calendar.loc['Earnings Date']
                            if isinstance(earnings_date, pd.Series):
                                earnings_date = earnings_date.iloc[0] if len(earnings_date) > 0 else None
                            
                            earnings_data[ticker]["next_earnings"] = earnings_date.strftime('%Y-%m-%d') if pd.notna(earnings_date) else None
                            earnings_data[ticker]["revenue_estimate"] = calendar.loc['Revenue Estimate'].iloc[0] if 'Revenue Estimate' in calendar.index else None
                            earnings_data[ticker]["earnings_estimate"] = calendar.loc['Earnings Estimate'].iloc[0] if 'Earnings Estimate' in calendar.index else None
                        else:
                    ...
                    # Check if it's a dictionary
                        ...
                        if 'Earnings Date' in calendar:
                            ...
                            earnings_data[ticker]["next_earnings"] = earnings_date.strftime('%Y-%m-%d') if earnings_date and pd.notna(earnings_date) else None
                            earnings_data[ticker]["revenue_estimate"] = calendar.get('Revenue Estimate', None)
                            earnings_data[ticker]["earnings_estimate"] = calendar.get('Earnings Estimate', None)
                        else:
                            earnings_data[ticker]["next_earnings"] = None
                    else:
                        earnings_data[ticker]["next_earnings"] = None
                ...
                # If calendar access fails, try alternative method
                try:
                    # Try using earnings_dates property
                    ...
                    if earnings_dates is not None and not earnings_dates.empty:
                        ...
                        if not future_dates.empty:
                            ...
                            earnings_data[ticker]["next_earnings"] = next_date.strftime('%Y-%m-%d')
                        else:
                            earnings_data[ticker]["next_earnings"] = None
                    else:
                        earnings_data[ticker]["next_earnings"] = None
            ...
            try:
                ...
                if earnings_hist is not None and not earnings_hist.empty:
                    ...
                    if 'epsestimate' in earnings_hist.columns and 'epsactual' in earnings_hist.columns:
                        ...
                        earnings_data[ticker]["avg_surprise"] = float(surprises.mean())
            except:
                pass  # Historical earnings not critical
